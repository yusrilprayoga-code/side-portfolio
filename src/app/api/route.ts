...
async function processAIStream(context: string, prompt: string, model: string, options?: { maxTotalTokens?: number }) {
  ...
  const maxTotalTokens = options?.maxTotalTokens ?? Number(process.env.COHERE_DEFAULT_MAX_TOKENS) ?? 5000
  const modelMaxTokens = Number(process.env.COHERE_MODEL_MAX_TOKENS) || 5000
